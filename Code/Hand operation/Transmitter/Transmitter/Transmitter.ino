...
int            date[10];                                  // Создаём массив для приёма данных
...
void setup(){
    ...
    radio.begin();
    //Serial.begin(9600);                                 // Инициируем работу nRF24L01+
...
*/
   ...
   date[8]= kalAngleY;
   date[9]= kalAngleX;
   radio.write(&date, sizeof(date));                     // отправляем данные из массива data указывая сколько байт массива мы хотим отправить. Отправить данные можно с проверкой их доставки: if( radio.write(&data, sizeof(data)) ){данные приняты приёмником;}else{данные не приняты приёмником;}
   /* for(int i=0;i<5;i++)
   {
